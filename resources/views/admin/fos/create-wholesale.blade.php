...
<head>
    ...
    <meta charset="utf-8" />
    <title>Wholesale Order Sheet | {{ $business->name }}</title>
...
<body id="tc_body" class="header-fixed header-mobile-fixed subheader-enabled aside-enabled aside-fixed">
    ...
    <header class="pos-header bg-white">
        <h1 class="mt-2 mb-4 text-center text-success font-weight-bold">Order Sheet - Wholesale</h1>
    ...
    <div class="contentPOS">
        <div class="container-fluid">
            <form id="order-submit-form" action="{{ route('fos.store') }}" method="POST">
                @csrf
                <div class="row">
    ...
    <script>
        ...
        $("#barcode").keyup(function() {
            ...
            if (!isNaN(barcode)) {
                if (barcode.length == 4) {
                    //alert(barcode);
                    url = "{{ route('fos.barcode.cart.add.wholesale') }}";
                    $.ajax({
                        url: url,
                        type: "POST",
                        data: {
